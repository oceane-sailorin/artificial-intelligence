...
diag1 = [x+y for x,y in zip(rows,cols)]
diag2 = [x+y for x,y in zip(rows,cols [::-1])]
...
def naked_twins(values):
    ...
    for boxA in [box for box in values.keys() if len(values[box]) == 2]:
        for boxB in [box for box in peers[boxA] if len(values[box]) == 2]:
